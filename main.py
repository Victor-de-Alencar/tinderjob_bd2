...
def company_match():
    ...
    if request.method == "POST":
        if next_dev:  # Se uma empresa estiver disponível
            if 'skip' in request.form:
                return company_skip(next_dev.id)
            elif 'match' in request.form:
                if not check_if_match_exists(company_id, next_dev.id):
                    new_match = Match(company_id=company_id, dev_id=next_dev.id)
